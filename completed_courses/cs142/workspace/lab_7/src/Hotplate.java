...
public class Hotplate
{
    ...
    public static void main(String[] args) throws FileNotFoundException
    {
        ...
        do
        {
            ...
            myScan = new Scanner(System.in);//refresh scanner to clear out any previous values
            System.out.print("Please enter the name and location the file you would like to create minus its extension: ");
            answer = myScan.next();
    ...
    public static double[][] computeHotPlate(double[][] myDub, double levelOfAccuracy)
    {
        ...
        while(computeAgain == true)
        {
            ...
            for(outer = 1; outer < outerSize-1; outer++)
            {
                for(inner = 1; inner < innerSize-1; inner++)
                {
                    if(0.1 < ((rtrnDub[outer - 1][inner])+(rtrnDub[outer + 1][inner])+(rtrnDub[outer][inner - 1])+(rtrnDub[outer][inner + 1]))/4 - rtrnDub[outer][inner])
                    {
                        computeAgain = true;
                    }
                }
