...
LEDtones:
        ...
        call	#refineRand				; refine number down to between 0-3 and place in whichLEDTone
...
switchLoop:
        ...
        call    #getSwitch              ; wait for switch
  		call	#quarterDelay			; wait a little time to give a natural feel to game
        ...
        call    #success                ; pattern done replicated successfully
...
;*******************************************************************************
;   Determine which switch has been pressed and convert it to the same
;	format as out outLEDtone (i.e. instead of 1-4, use 0-3).
;
getSwitch:
        
        cmp.w	#1,&whichSwitch			; check if P2.0 has been tripped
          jeq	switchPress_1			; yes!
        cmp.w	#2,&whichSwitch			; check if P2.1 has been tripped
          jeq	switchPress_2			; yes!
        cmp.w	#4,&whichSwitch			; check if P2.2 has been tripped
          jeq	switchPress_3			; yes!
        cmp.w	#8,&whichSwitch			; check if P2.3 has been tripped
          jeq	switchPress_4			; yes!
...
;*******************************************************************************
;	Port 1.2 (reset switch) ISR
;

P1_ISR:
		bic.b   #0x04,&P1IFG            ; P1.2 IFG Cleared
		mov.w	#restartGame,0x0002(SP)	; return from interrupt to restartGame
		bic.w   #LPM0,0x0000(SP)       	; make sure to exit LPM0
		reti

;*******************************************************************************
;   Port 2 (switches) ISR
;	Turns off low power mode, moves the value of P2IFG into whichSwitch which will be used
;	in getswitch routine. Clear P2IFG to break interrupt. Return to main code.
;
P2_ISR:
		bic.w   #LPM0,0x0000(SP)       	; exit LPM0 (but keep interrupts on)
        mov.b	P2IFG,&whichSwitch		; use whichSwitch to save button press
        bic.w	#0xfff0,&whichSwitch	; clear everything but the bottom four bits of whichSwitch
        bic.b	#0x0f,&P2IFG			; Clear IFG
        bic.b   #0x0f,&P2IE             ; Disable interrupts in P2.0-3 
        reti						
...
;*******************************************************************************
        ...
        .sect	".int02"				; P1 vector section
        .word	P1_ISR					; address of P1 ISR
