...
int LoadShader(const std::string &pfilePath_vs, const std::string &pfilePath_fs, bool bindTexCoord0, bool bindNormal, bool bindColor, GLuint &shaderProgram, GLuint &vertexShader, GLuint &fragmentShader)
{
        shaderProgram=0;
        vertexShader=0;
        fragmentShader=0;
        ...
        if(vertexShaderString.empty() or fragmentShaderString.empty()) return -1;
...
int main (int argc, char* argv[])
{
        ...
        //We don't need any extensions. Useless code. Comment out for now.
        /*
        std::cout << "List of available extensions:\n";
        for(int i=0; i<NumberOfExtensions; i++)
        {
                const GLubyte *ccc=glGetStringi(GL_EXTENSIONS, i);
                std::cout << "\t" << ccc << std::endl;
        }
        std::cout << "End of available extensions.\n";
        */

        InitGLStates();
 
        if(LoadShader("Shader1.glvs", "Shader1.glfs", false, false, true, ShaderProgram[0], VertexShader[0], FragmentShader[0])==-1)
        {
                exit(1);
        }
        else
        ...
        if(LoadShader("Shader2.glvs", "Shader2.glfs", true, true, false, ShaderProgram[1], VertexShader[1], FragmentShader[1])==-1)
        {
                exit(1);
        }
        else
        ...
        {
            std::cout << "GLUT main loop exit.";
        }
        return 0;
}

int objtest (int argc, char* argv[])
{
    objread o("test.obj");

    std::vector<position> pts = o.getVerts();
    std::vector<texcoords> tex = o.getTexCoords();
    std::vector<triangle> tris = o.getTriangles();
    
    std::vector<position>::iterator piter;
    std::vector<texcoords>::iterator txiter;
    std::vector<triangle>::iterator triter;
    ...
    for(txiter = tex.begin(); txiter != tex.end(); ++txiter)
    {
        std::cout << *txiter << std::endl;
    }

    for(triter = tris.begin(); triter != tris.end(); ++triter)
    {
        std::cout << *triter << std::endl;
    }
