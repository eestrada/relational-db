...
STACK			.equ	0x0280				; set value to initialize stack pointer with
...
portOutputSet1	.equ	0x01				; set portOutputSet constant
portToggleSet1	.equ	0x01				; set portToggleSet constant
...
RESET:      mov.w   #STACK,SP				; initialize stack pointer
            ...
            bis.b	#portOutputSet1,&P1DIR  ; set P1.0 as output
            bic.b   #portToggleSet1,&P1OUT  ; turn P1.0 off
...
mainloop:   call	#fiveSecondsOn			; call subroutine "fiveSecondsOn"
			...
			jmp		mainloop				; begin process again
...
fiveSecondsOnSlave:							; delays for five seconds, one second at a time
			...
			ret								; return to address pointed to by stack
...
toggleLight:
			xor.b   #portToggleSet1,&P1OUT  ; toggle P1.0
			ret								; return to address pointed to by stack
...
quarterSecondDelay:							; waste spproximately 95000 clock cycles. This will take about one quarter of a second.
